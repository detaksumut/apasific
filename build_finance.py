          ...
          <div class="journal-main-content" style="display: flex; flex-direction: column; gap: 40px;" data-aos="fade-right">
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 35px;">
              ...
              <p style="text-align: justify; line-height: 1.8; color: var(--text); margin-bottom: 0;">
                The Finance, Banking &amp; Investment discipline under the Association of Asia Pacific Academician (ASIA) provides an academic platform for researchers, academics, professionals, bankers, financial practitioners, and policymakers to publish scholarly works in the fields of finance, banking, and investment. This discipline encourages scientific contributions that support financial innovation, sustainable investment, and the development of the financial sector at national, regional, and international levels.
              </p>
            </div>
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 35px;">
              ...
              <ul class="values-list" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px 24px; padding-left: 5px;">
                ...
                <li style="grid-column: span 2;">Islamic Finance</li>
              </ul>
            </div>
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; box-shadow: var(--shadow);">
              ...
              <div style="padding: 25px; font-family: 'Inter', sans-serif;">
                <h3 style="font-size: 16px; font-weight: bold; color: var(--white); margin-bottom: 10px; line-height: 1.4;">
                  Jurnal Keuangan, Perbankan dan Investasi
                </h3>
                <p style="font-size: 12px; color: var(--text-muted); line-height: 1.6; margin-bottom: 20px;">
                  Fokus pada kajian ilmu manajemen keuangan, perbankan, investasi, pasar modal, teknologi keuangan (FinTech), manajemen risiko, dan keuangan syariah.
                </p>
...
# Assemble finance.html
finance_html = header_part + finance_main_content + footer_part

# Sync navbar and style.css cache globally to v=29
finance_html = re.sub(r'style\.css\?v=\d+', 'style.css?v=29', finance_html)

with open('finance.html', 'w', encoding='utf-8') as f:
    f.write(finance_html)
print("finance.html created successfully.")

print("Updating index.html link references...")
with open('index.html', 'r', encoding='utf-8') as f:
    idx_content = f.read()

# Update dropdown menu for finance
idx_content = idx_content.replace('<a href="#div-finance"><span class="dd-icon">◈</span> Finance, Banking &amp; Investment</a>',
                                  '<a href="finance.html"><span class="dd-icon">◈</span> Finance, Banking &amp; Investment</a>')

# Update homepage card link for finance
start_idx = idx_content.find('<div class="division-card" id="div-finance"')
if start_idx != -1:
    link_idx = idx_content.find('href="#contact"', start_idx)
    if link_idx != -1:
        idx_content = idx_content[:link_idx] + 'href="finance.html" class="div-link">Explore Division →</a>' + idx_content[idx_content.find('</a>', link_idx)+4:]
...
if nav_match:
    ...
    files = [
        ...
        'quality.html', 'academy.html', 'young.html', 'awards.html', 'research.html', 'accounting.html', 'business.html', 'finance.html'
    ]
    for fn in files:
        if os.path.exists(fn):
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                # Bump cache to v=29
                new_content = re.sub(r'style\.css\?v=\d+', 'style.css?v=29', new_content)
    ...
    idx_content = re.sub(r'style\.css\?v=\d+', 'style.css?v=29', idx_content)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(idx_content)
