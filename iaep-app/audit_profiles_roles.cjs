...
async function main() {
  ...
  // Mirrors isAdminRole / normalizeRole from src/lib/roles.ts
  const ROLE_MAP = {
  ...
  // ── 1. Fetch ALL profiles (SELECT only) ────────────────────────────────
  ...
  // Active/Approved users whose role is null — Fix A would not help them
  ...
  if (adminRows.length === 0) {
    console.log('  (none found — this is the likely cause of persistent 403)');
  } else {
  ...
  console.log('\n── UNKNOWN ROLE STRINGS (set but unmappable by normalizeRole) ──');
