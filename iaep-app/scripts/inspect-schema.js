...
async function inspectSampleSubmission() {
  const { data } = await supabase.from('submissions').select('*').limit(1).single();
  console.log('Submission schema keys:', Object.keys(data));
}

inspectSampleSubmission();
