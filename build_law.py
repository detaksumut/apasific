...
law_main_content = """
    ...
    <header class="page-hero">
      ...
      <div class="container page-hero-content">
        ...
        <h1 class="page-title" style="font-size: 3rem; margin-bottom: 12px; font-family: var(--font-cinzel);" data-aos="fade-up" data-aos-delay="100">Law, Governance &amp; Public Administration</h1>
        ...
        <p class="page-subtitle" style="font-size: 1.1rem; font-style: italic; max-width: 800px; margin: 0 auto; color: var(--text-muted);" data-aos="fade-up" data-aos-delay="300">Promoting Justice, Good Governance, and Public Accountability</p>
      </div>
    </header>
          ...
          <div class="journal-main-content" style="display: flex; flex-direction: column; gap: 40px;" data-aos="fade-right">
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 35px;">
              ...
              <p style="text-align: justify; line-height: 1.8; color: var(--text); margin-bottom: 0;">
                The Law, Governance &amp; Public Administration discipline under the Association of Asia Pacific Academician (ASIA) provides an academic platform for researchers, academics, legal practitioners, public administrators, policymakers, and professionals to publish scholarly works in the fields of law, governance, and public administration. This discipline encourages scientific contributions that support justice, transparency, accountability, and effective public governance at national, regional, and international levels.
              </p>
            </div>
          ...
          <div class="journal-sidebar" style="position: sticky; top: 110px; display: flex; flex-direction: column; gap: 30px;" data-aos="fade-left">
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; box-shadow: var(--shadow);">
              ...
              <div style="position: relative; height: 180px; overflow: hidden; border-bottom: 1px solid var(--border);">
                <img src="public/law_journal_cover.png" alt="Journal Cover" style="width: 100%; height: 100%; object-fit: cover;" />
              ...
              <div style="padding: 25px; font-family: 'Inter', sans-serif;">
                <h3 style="font-size: 16px; font-weight: bold; color: var(--white); margin-bottom: 10px; line-height: 1.4;">
                  Jurnal Hukum, Tata Kelola dan Administrasi Publik
                </h3>
                <p style="font-size: 12px; color: var(--text-muted); line-height: 1.6; margin-bottom: 20px;">
                  Fokus pada kajian ilmu hukum, administrasi publik, tata kelola pemerintahan, kepatuhan, anti-korupsi, dan tata kelola yang baik.
                </p>
...
# Assemble law.html
law_html = header_part + law_main_content + footer_part

# Sync navbar and style.css cache globally to v=32
law_html = re.sub(r'style\.css\?v=\d+', 'style.css?v=32', law_html)

with open('law.html', 'w', encoding='utf-8') as f:
    f.write(law_html)
print("law.html created successfully.")

print("Updating index.html link references...")
with open('index.html', 'r', encoding='utf-8') as f:
    idx_content = f.read()

# Update dropdown menu for law
idx_content = idx_content.replace('<a href="index.html#div-law"><span class="dd-icon">◈</span> Human Resources &amp; Organizational Development</a>',
                                  '<a href="hr.html"><span class="dd-icon">◈</span> Human Resources &amp; Organizational Development</a>') # fix HR links if missing
idx_content = idx_content.replace('<a href="index.html#div-law"><span class="dd-icon">◈</span> Law &amp; Governance</a>',
                                  '<a href="law.html"><span class="dd-icon">◈</span> Law, Governance &amp; Public Administration</a>')

# Update homepage card link for law
start_idx = idx_content.find('<div class="division-card" id="div-law"')
if start_idx != -1:
    link_idx = idx_content.find('href="#contact"', start_idx)
    if link_idx != -1:
        idx_content = idx_content[:link_idx] + 'href="law.html" class="div-link">Explore Division →</a>' + idx_content[idx_content.find('</a>', link_idx)+4:]
...
if nav_match:
    ...
    files = [
        ...
        'quality.html', 'academy.html', 'young.html', 'awards.html', 'research.html', 'accounting.html', 'business.html', 'finance.html', 'hr.html', 'economics.html', 'law.html'
    ]
    for fn in files:
        if os.path.exists(fn):
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                # Bump cache to v=32
                new_content = re.sub(r'style\.css\?v=\d+', 'style.css?v=32', new_content)
    ...
    idx_content = re.sub(r'style\.css\?v=\d+', 'style.css?v=32', idx_content)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(idx_content)
